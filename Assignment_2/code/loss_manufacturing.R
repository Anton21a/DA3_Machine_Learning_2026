...
setwd("C:/Users/user/Desktop/MA2y/MA1y/Prediction_with_ML/Machine-Learning/Assignment 3")
...
final_threshold <- best_thresholds[[final_model]]
final_threshold

data_holdout$rf_pred_class <- ifelse(
  data_holdout$rf_prob >= final_threshold,
  "yes",
  "no"
)

data_holdout$rf_pred_class <- factor(
  data_holdout$rf_pred_class,
  levels = c("no", "yes")
)


confusion_rf <- confusionMatrix(
  data = data_holdout$rf_pred_class,
  reference = data_holdout$fast_growth_f,
  positive = "yes"
)
...
conf_table <- table(
  Actual = data_holdout$fast_growth_f,
  Predicted = data_holdout$rf_pred_class
)

conf_table
library(knitr)
kable(conf_table, caption = "Confusion matrix (RF, holdout sample)")
